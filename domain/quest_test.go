...
func TestAwardQuestReward(t *testing.T) {
	t.Parallel()
	now := time.Now()
	pet := domain.New(uuid.New(), now)

	progress := pet.AwardQuestReward(0, now)
	assert.Equal(t, 0, progress.XPGranted)

	progress = pet.AwardQuestReward(-5, now)
	assert.Equal(t, 0, progress.XPGranted)


	progress = pet.AwardQuestReward(10, now)
	assert.Equal(t, 13, progress.XPGranted)
}
